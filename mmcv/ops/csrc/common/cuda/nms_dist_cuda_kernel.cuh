...
__device__ inline bool devDist(float const *const a, float const *const b,
                              const int offset, const float threshold) {
  float xdist = powf(a[0] - b[0], 2);
  float ydist = powf(a[1] - b[1], 2);
  float dist = sqrtf(xdist+ydist);

  return dist < threshold;
}

__global__ void nms_dist_cuda(const int n_boxes, const float iou_threshold,
  ...
  CUDA_2D_KERNEL_BLOCK_LOOP(col_start, blocks, row_start, blocks) {
    ...
    if (tid < row_size) {
      ...
      for (i = start; i < col_size; i++) {
        if (devDist(cur_box, block_boxes + i * 4, offset, iou_threshold)) {
          t |= 1ULL << i;
        }
      }
...
#endif  // NMS_DIST_CUDA_KERNEL_CUH
